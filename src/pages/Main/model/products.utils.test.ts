...
test("filterProducts applies category, search, and sorting in a stable order", () => {
  ...
  assert.deepEqual(
    result.map((product) => product.title),
    ["Alpha Jacket", "Atlas Coat"],
  );
});

test("paginateProducts clamps the page and deterministic stock stays stable", () => {
  const pagination: PaginationStateType = {
    currentPage: 3,
    itemsPerPage: 2,
  };

  const { items, pagination: normalizedPagination } = paginateProducts(
    sampleProducts,
    pagination,
  );
